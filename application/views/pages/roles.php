...
/* Fn init */
var createRole = null, dtRoleTable = null, deleteRole = null;
	
window.addEventListener('load', function() {

(function($) {

dtRoleTable = $('#role_table').DataTable({
	ajax: {
		url: "<?php echo site_url('users/roles/list', SITE_SCHEME) ?>"
	},
	columns: [
		{
			title: "Name",
		...
		},
		{
			...
			render: function(data) {
				...
				edtBtnEl.attr({
					onclick: "PageOverlay.show(); location.href='<?php echo site_url('users/roles/edit') ?>/" + data + "';"
				}).appendTo(btnCnt);
				var delBtnEl = $('<button type="button" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></button>');
				delBtnEl.attr({
					onclick: "if (confirm('Delete role?')) location.href='<?php echo site_url('users/roles/delete') ?>/" + data + "';"
				}).appendTo(btnCnt);
...
createRole = function() {
	location.href = "<?php echo site_url('users/roles/create') ?>";
}
